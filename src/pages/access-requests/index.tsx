...
import { getAccessRequests, deleteAccessRequestById } from 'apiSdk/access-requests';
import { AccessRequestInterface } from 'interfaces/access-request';

type ColumnType = ColumnDef<AccessRequestInterface, unknown>;

interface AccessRequestListPageProps {
...
export function AccessRequestListPage(props: AccessRequestListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AccessRequestInterface>>(
    () => `/access-requests?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteAccessRequestById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: AccessRequestInterface) => {
    if (hasAccess('access_request', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/access-requests/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/access-requests/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
          ...
          {hasAccess('access_request', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/access-requests/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('access_request', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="access_request" />
        </Flex>
        {hasAccess('access_request', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/access-requests/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
